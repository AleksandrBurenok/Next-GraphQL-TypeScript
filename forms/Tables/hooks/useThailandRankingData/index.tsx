...
import { getThailandRanking } from 'queries/ranking';

import { Sections } from 'enums/path';
import { Keys } from 'enums/ranking';

import { ALL } from 'constants/selectList';
import { leaguesThRankingId } from 'constants/leaguesThRankingId';
...
const useRankingData = ({ priorityLeagues, watch, section }: Props) => {
  ...
  const getData = useCallback(() => {
    setIsLoading(true);
    getThailandRanking(params).then((data) => {
  ...
  const filteredRanking = !!ranking.length
    ? ranking.filter((entity) => leaguesThRankingId.includes(entity.id))
    : [];
